...
if (empty ($errormsg)) {
  ...
  if (is_admin ()) {
    ...
    if ("On Shelf" == $resource["status"]) {
      $checkout_link = "<a href=\"checkout1.php?id=" . $HTTP_GET_VARS["id"] . "\">Check-out Resource</a>";
    }
    else {
      $checkout_link = "You can only check-out books that have an 'on shelf' status.";
    }

    $update_link = "<a href=\"update1.php?id=" . $HTTP_GET_VARS["id"] . "\">Update Record</a>";
